...
- [x] **I/O intrinsics** — `print()` (no newline), `println_int()`, `print_int()`, `println_bool()`, `print_bool()`
- [ ] **`for`-loop desugar over `Iterator`** — the `core::iter` `Iterator` aspect (`type Item`, `next(self<their<Self>>)<Option<Self::Item>>`) is shipped and consumable via explicit `.next()` / `match` loops (protocol proven for `Int`/`Float` in `test/modules/test_iterator_protocol.vyb`). Deciding `for (item in col)` to repeated `next()` calls is queued: it needs a type-aware transform (parse-time can't distinguish a Vec from a user iterator), plus handling of `break`/`continue` and payload extraction. Binding `Vec<T>`/`HashMap<K,V>`/`HashSet<T>` as iterators is also blocked on calling `len`/`get` on a nested `their<Vec<T>>` struct field (currently unresolved for both generic and concrete receivers).
