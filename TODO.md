...
- [ ] **`vyb bindgen` (full, v0.6+)** — libclang-based C parsing (preprocessor/macros, bitfields). Array-parameter shape is done (`T a[]`/`T a[N]` → `T*`: `char s[]` → `CString`, `double vals[]` → `loc<CDouble>`; `arrayparams.h`/`test_arrayparams_bindings.vyb`), and function-pointer params (inline and typedef'd) bind as `fn(...) -> ...` types (`fnpointer.h`/`test_fnpointer_bindings.vyb`). Calling a C callback with a Vyb function is not ABI-supported yet — `fn` lowers to `{ptr,ptr}`, C expects a bare code pointer — and needs thunking (per the monomorphization note, C FFI remains the escape hatch for callback-style interfaces). Note: C-like enum variants are distinct typed values backed by a scalar `i64` tag, so an extern function taking a C enum by value can type the param as the enum name and interoperates (see `test/ffi/enum_by_value.vyb`). Values expose their positional (variant) tag as an `Int` via `.tag` (`test/units/test_enum_tag.vyb`).
