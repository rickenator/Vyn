...
- [x] **I/O intrinsics** — `print()` (no newline), `println_int()`, `print_int()`, `println_bool()`, `print_bool()`
- [ ] **`for`-loop desugar over `Iterator`** — the `core::iter` `Iterator` aspect (`type Item`, `next(self<their<Self>>)<Option<Self::Item>>`) is shipped and consumable via explicit `.next()` / `match` loops. **Nested `their<Vec<T>>` field resolution is done** (`test/modules/test_nested_their_vec_field.vyb`). **Generic bind `Option<T>` is done** — `match`/`select` on a generic-bind-returned `Option` now materializes the concrete enum (variant patterns `Some(x)`/`None` resolve), and bare `Some(x)`/`None` inside a monomorphized generic bind substitute the active type params (both previously failed with `Option_T` / "Unknown type identifier: Some"). This unblocks the generic collection iterator: `import collections` now ships `VecIter<T>` + `v.iter()` (a cursor iterator over `Vec<T>`, generic over the element type), covering `test/modules/test_vec_iter.vyb`. What remains for `for (item in col)` desugaring is the type-aware transform itself (parse-time can't distinguish a Vec from a user iterator; needs a semantic/codegen-level transform plus `break`/`continue` and payload extraction), and waking up `HashMap`/`HashSet` iterator binds similarly.
