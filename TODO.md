...
- [ ] **`vyb bindgen` (full, v0.6+)** — libclang-based C parsing (preprocessor/macros, function pointers, bitfields, array-as-param shape). Note: C-like enum variants are distinct typed values backed by a scalar `i64` tag, so an extern function taking a C enum by value can type the param as the enum name and interoperates (see `test/ffi/enum_by_value.vyb`). Values expose their positional (variant) tag as an `Int` via `.tag` (`test/units/test_enum_tag.vyb`).
...
- [x] **I/O intrinsics** — `print()` (no newline), `println_int()`, `print_int()`, `println_bool()`, `print_bool()`
- [ ] **`for`-loop desugar over `Iterator`** — the `core::iter` `Iterator` aspect (`type Item`, `next(self<their<Self>>)<Option<Self::Item>>`) is shipped and consumable via explicit `.next()` / `match` loops (protocol proven for `Int`/`Float` in `test/modules/test_iterator_protocol.vyb`). Deciding `for (item in col)` to repeated `next()` calls is queued: it needs a type-aware transform (parse-time can't distinguish a Vec from a user iterator), plus handling of `break`/`continue` and payload extraction. **Nested `their<Vec<T>>` field resolution is done** — a struct field typed `their<Vec<T>>` (e.g. `VecIter<T>` holding the Vec by reference) now resolves all built-in Vec methods (`len`/`get`/`set`/`push`), with reads and mutations reaching the borrowed backing Vec (`test/modules/test_nested_their_vec_field.vyb`), and a concrete `Iterator` bind over such a field works (`next` advancing its own cursor). The remaining blocker for **generic** `Vec<T>`/`HashMap<K,V>`/`HashSet<T>` iterator binds is incoming: constructing/`match`-ing `Option<T>` from a type-parameter payload (`Option<T>::Some(x)` / bare `Some(x)`) inside a generic bind body does not type-resolve at the call site (the concrete case works — see `MapOps.get`), tracked separately under generic enum binding.
