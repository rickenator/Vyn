...
- [x] **`vyb bindgen` (MVP)** — `vyb bindgen <header.h> [-o out.vyb]` parses a C subset (typedefs, `struct`/`enum` declarations, scalar/pointer types, trailing `...` varargs) and emits `share(all)` bodyless extern declarations + `#[repr(C)]` structs + enums. Functions and structs import and resolve against the host C ABI; covered by `test/bindgen/libsample.h` → `test/bindgen/libsample.vyb` + `test/bindgen/test_libsample_bindings.vyb`.
- [ ] **`vyb bindgen` (full, v0.6+)** — libclang-based C parsing (preprocessor/macros, function pointers, bitfields, array-as-param shape) and re-export of generated enums on `import` (enums currently do not cross module imports)
...
*Overall Status: ~60-65% complete toward 1.0 — 813 tests passing (harness, 100%)*
