...
- [ ] **`vyb bindgen` (full, v0.6+)** — libclang-based C parsing for the rest of the preprocessor (function-like/expression macros, `#include` expansion, conditional evaluation; object-like `#define` constants are already bound as shared constant functions — `preproc.h`/`test_preproc_bindings.vyb` — and bitfield structs are detected and skipped with a warning since they cannot be ABI-represented — `bitfields.h`/`test_bitfields_bindings.vyb`). Function-pointer params (inline and typedef'd) bind as bare `loc<fn(...) -> ...>` pointers (single `ptr` ABI), and a Vyb function can be passed to a C callback and called back through a stored fn pointer (`fnpointer.h`/`test_fnpointer_bindings.vyb`, `test/ffi/callback_fnptr.vyb`). Array-parameter shape is done: `T a[]`/`T a[N]` -> `T*` (`char s[]` -> `CString`, `double vals[]` -> `loc<CDouble>`; `arrayparams.h`/`test_arrayparams_bindings.vyb`). Note: C-like enum variants are distinct typed values backed by a scalar `i64` tag, so an extern function taking a C enum by value can type the param as the enum name and interoperates (see `test/ffi/enum_by_value.vyb`). Values expose their positional (variant) tag as an `Int` via `.tag` (`test/units/test_enum_tag.vyb`).
