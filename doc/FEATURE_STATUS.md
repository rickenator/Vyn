...
| Stdlib module foundation (`core`/`collections`/`io`) | 🚧 | Foundation + `core::math` helpers shipped. `core::aspects` pre-wires scalar binds; `core::math` layers `clamp`/`is_close` over the global math intrinsics. `io` documents the output intrinsics; `collections` now ships real `HashMap<K,V>` and `HashSet<K>` (`import collections`, by-ref bind methods, `test/modules/test_collections_hashmap.vyb`). File/network I/O, higher-order `Vec<T>`, and `Iterator` still track their compiler-feature items. `Option<T>`/`Result<T,E>` are built-in enums (transitional `core::option`/`core::result` retained for source-compat) |
