...
| Stdlib module foundation (`core`/`collections`/`io`) | 🚧 | Foundation + `core::math` helpers shipped. `core::aspects` pre-wires scalar binds; `core::math` layers `clamp`/`is_close` over the global math intrinsics. `collections` ships real `HashMap<K,V>` and `HashSet<K>` (`import collections`, by-ref bind methods, with a hash-bucket (chained) key index built on `Vec`; `test/modules/test_collections_hashmap.vyb`). `io` now ships real File I/O — `File { fd, path }`, `open` + `open_read`/`open_write`/`open_append`, `close`, `write_str`, `read_all`, `error_code`/`error_message`, and `FILE_*` mode helpers over the runtime `__vyb_file_*` intrinsics (`test/modules/test_file_io.vyb`). Network I/O, higher-order `Vec<T>`, and `Iterator` still track their compiler-feature items. `Option<T>`/`Result<T,E>` are built-in enums (transitional `core::option`/`core::result` retained for source-compat) |
