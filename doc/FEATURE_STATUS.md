...
| `chan<T>` (typed channels) | ✅ | Built-in generic thread-safe channel (single i64 handle, spatially identical to Int, so sharing a chan by value across pthreads references one channel). `chan<T>()` / `chan<T>(cap)` construct an unbounded/bounded channel; methods `send(v)`, `recv()` (blocking), `poll()` (non-blocking), `len()`, `free()`, and `handle()` (the raw Int for `chan_select`). Int-family scalar payloads use the int-slot channel runtime; String payloads use the refcounted string runtime with reference transfer (`test/modules/test_chan_typed.vyb`, `test/modules/test_chan_threaded.vyb`). Float/Bool/Char payloads and non-identifier method receivers remain follow-ups |
