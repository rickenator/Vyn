...
| Structs | ✅ | |
| Enums | ✅ | C-like integer enums map variants to sequential `i64` constants with `Enum::Variant` access; data-carrying (tagged-union) enums like `enum Shape { Circle(Float), Rect(Float, Float), Unit }` compile to a value-semantics `{ i64 tag, [N x i8] data }` union, construct via `Shape::Variant(args)`, and match on variants (`Circle(r) ->`, `Unit ->`) binding payload fields; a match covering every variant is treated as exhaustive so an all-return final-statement match compiles cleanly. Generic data enums, `select` variant destructuring, `Option`/`Result` as built-in enums, and semantic rejection of non-exhaustive matches remain for v0.6 |
