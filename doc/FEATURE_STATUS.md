...
| Structs | ✅ | |
| Enums | ✅ | C-like integer enums map variants to sequential `i64` constants with `Enum::Variant` access; data-carrying (tagged-union) enums like `enum Shape { Circle(Float), Rect(Float, Float), Unit }` compile to a value-semantics `{ i64 tag, [N x i8] data }` union, construct via `Shape::Variant(args)`, and match on variants (`Circle(r) ->`, `Unit ->`) binding payload fields; matches must be exhaustive, and an exhaustive match's no-match default is unreachable so an all-return final-statement match compiles cleanly. Generic data enums, `select` variant destructuring, and `Option`/`Result` as built-in enums remain for v0.6 |
