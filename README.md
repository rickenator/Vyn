...
3. ✅ **Lambda/Closure Codegen** (landed): lambdas lower to a uniform closure value (`struct { env, fn }`); free variables are captured by value into a heap-allocated environment. Capturing closures work standalone, returned from functions, and as `fn` arguments to the Vec `map`/`filter`/`reduce` combinators. `|x<Int>| -> x * 2`. Move / mutable / `our` capture remain for v0.6+.
