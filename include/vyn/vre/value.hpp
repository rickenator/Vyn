...
// VreObject: represents a struct/class instance
struct VreObject {
    // For now, use a map of field names to values. Optimize later for index-based access.
    std::unordered_map<std::string, VreValue> fields;
    // Type info: could be a string, type id, or pointer to a type descriptor
    std::string type_name; // Or use a TypeId or pointer to a type descriptor struct
    // Reference count for manual memory management (if not using shared_ptr everywhere)
    // If using shared_ptr, refcount is managed automatically, but you can add a custom counter for debugging or GC
    // int refcount = 1; // Uncomment if you want manual refcounting
};

// VreArray: represents a dynamic array
struct VreArray {
    std::vector<VreValue> elements;
    // Optionally, store element type info for runtime type checking
    std::string element_type_name; // Or TypeId, etc.
    // Reference count (see above)
    // int refcount = 1;
};
...
#ifndef VYN_VRE_MEM_DEBUG
#define VYN_VRE_MEM_DEBUG 1
#endif
