...
class Generator {
public:
    Generator(CXTranslationUnit tu, CXFile mainFile, const std::string& ns)
        : tu_(tu), mainFile_(mainFile), ns_(ns.empty() ? "HeaderConsts" : ns) {}
    ...
    std::string run(std::vector<std::string>& warnings) {
        ...
        if (!intMacros_.empty()) {
            ...
            // `Preproc`), accessed as `Preproc::MAX_BUFSIZE`.
            os << "share(all)\nenum " << ns_ << " {\n";
            size_t n = intMacros_.size();
            for (size_t i = 0; i < n; ++i) {
                os << "    " << intMacros_[i].first << " = " << intMacros_[i].second;
                if (i + 1 < n) os << "\n";
            }
            os << "\n}\n\n";
        }
    ...
    void emitConstMacro(const std::string& name, const std::string& text,
        ...
        if (type == "CInt" || type == "Int") {
            intMacros_.push_back({name, text});
            return;
        }
        os << "share(all)\n" << name << "()<" << type << "> -> {\n    return " << text
           << "\n}\n\n";
    ...
    std::vector<std::pair<std::string, std::string>> intMacros_;
    std::string ns_;
...
std::string generateBindingsFull(const std::string& headerPath,
    ...
    Generator gen(tu, mainFile, deriveNamespace(headerPath));
