...
static int mod_fetch_github(const std::string& ownerRepoPath, const std::string& exePath, std::string& outDir) {
    ...
    {
        std::ostringstream d;
        d << "# generated by vyb mod install (github transport, #175). Integrity is the\n"
          << "# sha256 pin (TOFU) recorded in vyb.lock, not the TLS transport; the\n"
          << "# verified-TLS client handshake has an interop gap against CDNs (#175).\n"
          << "import https::{https_get_full}\n"
          << "import http::{HttpResponse}\n"
          << "import io::{open_write, write_str, close}\n"
          << "main()<Int> -> {\n"
          << "    r<HttpResponse> = https_get_full(\"raw.githubusercontent.com\", 443, \"" << rawPath << "\")\n"
