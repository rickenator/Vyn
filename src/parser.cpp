...
std::unique_ptr<vyn::Module> Parser::parse_module() { 
    ...
    if (this->pos_ < this->tokens_.size() && this->tokens_[this->pos_].type != vyn::TokenType::END_OF_FILE) { 
        size_t final_check_pos = this->pos_;
    ...
    } else if (this->pos_ >= this->tokens_.size() && !this->tokens_.empty() && this->tokens_.back().type != vyn::TokenType::END_OF_FILE) { 
